...
--
-- Definition for table Proprietário
--
...
CREATE TABLE proprietario (
  ...
  pro_var_telefone VARCHAR(14)  NULL COMMENT 'Telefone',
  pro_dti_cadastro TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Cadastro',
  PRIMARY KEY (pro_int_codigo),
  INDEX IDX_proprietario_pro_dti_cadastro (pro_dti_cadastro),
...
BEGIN
  ...
  -- VALIDAÇÕES
  IF p_rac_var_raca IS NULL THEN
  ...
  INTO v_existe
  FROM raca
  WHERE rac_var_raca = p_rac_var_raca;
  IF v_existe THEN
    SET p_msg = concat(p_msg, 'Já exista racça cadastrada com este nome.<br />');
  END IF;
  ...
  IF p_msg = '' THEN
    ...
    INSERT INTO raca (rac_var_raca)
    VALUES (p_rac_var_raca);
...
BEGIN
  ...
  IF v_existe THEN
    SET p_msg = concat(p_msg, 'Já existe raça com este nome cadasrada.<br />');
  END IF;
  ...
  IF p_msg = '' THEN
    ...
    UPDATE raca
    SET rac_var_raca = p_rac_var_raca
    WHERE rac_int_codigo = p_rac_int_codigo;
...
--
DROP PROCEDURE IF EXISTS sp_raca_del$$
CREATE PROCEDURE `sp_raca_del` (IN `p_rac_int_codigo` INT(11), INOUT `p_status` BOOLEAN, INOUT `p_msg` TEXT)  
  SQL SECURITY INVOKER
  COMMENT 'Procedure de Delete'
BEGIN
